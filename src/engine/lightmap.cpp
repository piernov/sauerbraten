...
static uint generatelumel(lightmapworker *w, const float tolerance, uint lightmask, const vector<const extentity *> &lights, const vec &target, const vec &normal, vec &sample, int x, int y)
{
    ...
    switch(w->type&LM_TYPE)
    {
        case LM_BUMPMAP0:
            ...
            extern vec orientation_tangent[8][3];
            extern vec orientation_bitangent[8][3];            
...
static inline void generatealpha(lightmapworker *w, float tolerance, const vec &pos, uchar &alpha)
{
    ...
    if(w->slot->layermask)
    {
        ...
        if((w->rotate&5)==1 || w->rotate>=6) swap(s, t);
        if((w->rotate>=2 && w->rotate<=4) || w->rotate==7) s = -s;
        if((w->rotate>=1 && w->rotate<=2) || w->rotate==5 || w->rotate==7) t = -t;
...
{
    bool flipx = (rotate>=2 && rotate<=4) || rotate==7,
         flipy = (rotate>=1 && rotate<=2) || rotate==5 || rotate==7,
         swapxy = (rotate&5)==1 || rotate>=6;
